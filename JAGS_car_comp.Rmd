...
model {
 ...
 for (i in 1:nsites) {
    ...
    for (j in 1:nvisits) {
        ...
        logit.p[i, j] <- alpha0.car + alpha.cwd.car * CWD[i] 
        + alpha.precip.car * Precip[i, j] + alpha.block[Block[i]]
...
load("out.occ.car.comp-PsiCut-PCWDPrecBlock-200K150Kb.Rdata")
...
plot(
  ...
  xlim = c(0, 4),
  ylim = c(0, 1),
  main = paste0("Carabes du groupe \"compétiteur de salamandres\""),
